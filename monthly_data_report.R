# # # #test file
# d <- read_csv('test/sim_test_data_new_prepped_geocoder_3.3.0_score_threshold_0.5_census_block_group_0.6.0_2010.csv',
#               col_types = cols(INTAKE_ID = col_character(),
#                                                 SCREENING_DECISION = col_character(),
#                                                 DECISION_DATE = col_date(format = "%m/%d/%y"),#format = "%m/%d/%Y"),
#                                                 BIRTH_DATE = col_date(format = "%m/%d/%y"),#format = "%m/%d/%Y"),
...
path <- "/tmp/"
write.csv(d_csv, paste(path, "monthly_report_v5.0.1.csv", sep = ''))
